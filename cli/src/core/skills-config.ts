...
// ============================================================================
// MIGRATION: cli/skills.config.json → ~/.skills/config.json
...
function migrateProjectConfig(projectConfigPath: string): string[] {
  try {
    ...
    log.info("Migrated skills.config.json → ~/.skills/config.json")
...
// LOAD EXCLUDED REFS
// Priority:
//   1. ~/.skills/config.json  (global user config, via readUserConfig())
